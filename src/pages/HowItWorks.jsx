    ...
    <div className="max-w-3xl mx-auto px-3 sm:px-4 py-6 sm:py-12">
      ...
      <div className="space-y-6 sm:space-y-8">
        <section>
          ...
          <div className="space-y-3 sm:space-y-4">
            {[
              ...
              ['4. Receive your USDT', 'Once the seller confirms they received your Kwacha, the smart contract automatically releases USDT to your wallet. The 0.8% fee is deducted from the released amount.'],
            ].map(([title, desc]) => (
        ...
        <section>
          ...
          <div className="space-y-3 sm:space-y-4">
            {[
              ...
              ['4. Smart contract releases', 'Once you confirm payment, the smart contract sends USDT directly to the buyer wallet. The 0.8% fee goes to the platform wallet.'],
            ].map(([title, desc]) => (
              <div key={title} className="rounded-xl border border-border bg-card p-4 sm:p-5">
                <h3 className="font-medium mb-1 text-sm sm:text-base">{title}</h3>
                <p className="text-xs sm:text-sm text-muted-foreground">{desc}</p>
        ...
        <section>
          ...
          <div className="rounded-xl border border-border bg-card p-4 sm:p-5">
            <p className="text-xs sm:text-sm text-muted-foreground">The escrow fee is 0.8% of the trade amount, deducted from the released USDT. This is collected by the smart contract and sent to the platform wallet automatically. No hidden charges, no subscription. You only pay when a trade completes.</p>
          </div>
        </section>
      </div>
