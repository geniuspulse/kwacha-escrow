    ...
    <nav className="sticky top-0 z-50 border-b border-border bg-background/80 backdrop-blur-lg">
      <div className="max-w-7xl mx-auto px-4 lg:px-6 h-16 flex items-center justify-between">
        <Link to="/" className="flex items-center gap-2">
          ...
          <span className="font-heading font-bold text-lg tracking-tight">Kwacha Escrow</span>
        </Link>
      ...
      {menuOpen && (
        <div className="md:hidden border-t border-border px-4 py-4 space-y-3 bg-background">
          <Link to="/trade" onClick={() => setMenuOpen(false)} className="block text-sm text-muted-foreground">Trade</Link>
          <Link to="/how-it-works" onClick={() => setMenuOpen(false)} className="block text-sm text-muted-foreground">How it works</Link>
          {user && <Link to="/dashboard" onClick={() => setMenuOpen(false)} className="block text-sm text-muted-foreground">Dashboard</Link>}
          {profile?.role === 'admin' && <Link to="/admin" onClick={() => setMenuOpen(false)} className="block text-sm text-muted-foreground">Admin</Link>}
          {user ? (
            <button onClick={handleSignOut} className="block text-sm text-destructive">Sign out</button>
          ) : (
            <div className="flex gap-3">
              <Link to="/login" onClick={() => setMenuOpen(false)} className="flex-1 text-center px-4 py-2 rounded-lg border border-border text-sm">Login</Link>
              <Link to="/register" onClick={() => setMenuOpen(false)} className="flex-1 text-center px-4 py-2 rounded-lg bg-primary text-primary-foreground text-sm font-semibold">Get Started</Link>
            </div>
          )}
        </div>
      )}
